...
def load_zarr_file_abs(zarr_path: str):
    try:
        # Open the Zarr hierarchy (root group)
        # 'r' mode is for read-only
        root_group = zarr.open(zarr_path, mode="r")
        ...
        actions = np.concatenate(
            (
                pose_ref_ds[:, :3, 3].reshape(-1, 3),
                pose_ref_ds[:, :3, :2].reshape(-1, 6),
                hand_ds[:, :],
            ),
            axis=1,
        )
        ...
        return {
            ...
            "initial_config": np.concatenate(
                (
                    pose_ds[0, :3, 3].reshape(1, -1),
                    pose_ds[0, :2, :3].reshape(1, -1),
                    hand_ds[0, :].reshape(1, -1),
                ),
                axis=1,
            )[0],
            "ee_pose": pose_ref_ds,
